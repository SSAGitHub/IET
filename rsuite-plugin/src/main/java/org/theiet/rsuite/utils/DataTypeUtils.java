...
public class DataTypeUtils {
	 ...
	 */
	public static String getDataTypeOptionValueLabel(
			ExecutionContext context,
			User user,
			String dataTypeName,
			String dataTypeOptionValueId) 
	throws RSuiteException {
		DataTypeOptionValue ov = getDataTypeOptionValue(context, user, dataTypeName, dataTypeOptionValueId);
	 ...
	 * @throws RSuiteException Thrown if unable to retrieve the specified information from RSuite.
	 */
	public static String getDataTypeOptionValueId(
			ExecutionContext context,
			User user,
			String dataTypeName,
			String dataTypeOptionValueLabel) 
	throws RSuiteException {
		DataTypeOptionValue ov = getDataTypeOptionValue(context, user, dataTypeName, dataTypeOptionValueLabel);
	 ...
	 * @param user
	 * @param dataTypeName Name of an existing data type.
	 * @param dataTypeOptionLabelOrId Either the data type option's label or ID.  The comparisons
	 * 		are case-insensitive.
	 * @return The data type option value from the specified data type matching the given criteria.
	 * @throws RSuiteException Thrown if unable to retrieve the specified information from RSuite.
	 */
	public static DataTypeOptionValue getDataTypeOptionValue(
			ExecutionContext context,
			User user,
			String dataTypeName,
			String dataTypeOptionLabelOrId) 
	throws RSuiteException {
		// Make sure the criteria is not null.  An empty or blank string could be fine.
		if (dataTypeOptionLabelOrId != null) {
			DataTypeManager dtm = context.getDomainManager().getCurrentDomainContext().getDataTypeManager();
			...
			for (DataTypeOptionValue ov : optionValueList) {
				if (dataTypeOptionLabelOrId.equalsIgnoreCase(ov.getLabel()) ||
						dataTypeOptionLabelOrId.equalsIgnoreCase(ov.getValue())) { 
					return ov;
				}
			}
		}
	...
	/**
	 * Get data type option values by data type name.
	 * 
	 * @param context
	 * @param user
	 * @param dataTypeName Name of an existing data type.
	 ...
	 * @throws RSuiteException Thrown if unable to retrieve the specified information from RSuite.
	 */
	public static DataTypeOptionValue[] getDataTypeOptionValues(
			ExecutionContext context,
			User user,
			String dataTypeName) 
	throws RSuiteException {
