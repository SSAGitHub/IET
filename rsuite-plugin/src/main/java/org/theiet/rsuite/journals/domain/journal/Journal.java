...
public class Journal {
	...
	private static final String LMD_FIELD_JOURNAL_ABBRV_TITLE = "journal_abbrv_title";
	...
	public Journal(ExecutionContext context, ContentAssembly journalCa) throws RSuiteException {
	...
	public Journal(ExecutionContext context, String journalCode, String journalCaId) throws RSuiteException {
		initializeFields(context, journalCode);
		journalCa = caService.getContentAssembly(user, journalCaId);
	}

	public Journal(ExecutionContext context, String journalCode) throws RSuiteException {
	...
	public void archiveAvailableArticles(ContentAssembly journalArchvie) throws RSuiteException {
		ContentAssemblyService caSvc = context.getContentAssemblyService();

		ObjectDetachOptions detachOpts = new ObjectDetachOptions();

		ContentAssembly articlePoolCa = ProjectBrowserUtils.getChildCaByType(context, journalCa,
				JournalConstants.CA_TYPE_ARTICLES);
		String articlePoolCaId = articlePoolCa.getId();

		Map<String, String> refIdMap = ProjectBrowserUtils.getCaRefIdMap(user, caSvc, articlePoolCa);

		if (journalArchvie != null) {
			List<ManagedObject> availableArticles = getAvailableArticles(context, user, journalCode);
			for (ManagedObject availableArticle : availableArticles) {

				ContentAssembly articleCa = caSvc.getContentAssembly(user, availableArticle.getId());
				String articleCaId = articleCa.getId();

				archiveArticle(journalArchvie, articleCa);

				caSvc.detach(user, articlePoolCaId, refIdMap.get(articleCaId), detachOpts);

			}
		}
	}

	public void archiveArticle(ContentAssembly articleToArchive) throws RSuiteException {
		ContentAssembly archiveCa = getArchiveCa();
		if (archiveCa == null) {
			throw new RSuiteException("Archive container doesn't exist for journal " + journalCode + " archiving: "
	...
	private void archiveArticle(ContentAssembly archiveCa, ContentAssembly articleToArchive) throws RSuiteException {

		String publishDateValue = articleToArchive.getLayeredMetadataValue(LMD_FIELD_ONLINE_PUBLISHED_DATE);

		Calendar publishDate = IetDate.parseDate(publishDateValue);
		ContentAssembly monthCa = getArchiveMonthCa(archiveCa, publishDate);
		caService.attach(user, monthCa.getId(), articleToArchive, new ObjectAttachOptions());
		IetUtils.removeMetaDataFieldFromCa(log, user, context.getManagedObjectService(), articleToArchive,
				LMD_FIELD_ARTICLE_AVAILABLE);
	}

	public void detachArticleFromArticlesPool(final ContentAssembly article) throws RSuiteException {

		ContentAssembly articlePool = ProjectBrowserUtils.getChildCaByType(context, journalCa,
				JournalConstants.CA_TYPE_ARTICLES);

		ContentAssemblyItem caRef = ProjectBrowserUtils.getFristCaRef(context, articlePool, article);
		caService.detach(user, articlePool.getId(), caRef.getId(), new ObjectDetachOptions());
	}

	private ContentAssembly getArchiveMonthCa(ContentAssembly journalArchvie, Calendar publishDate)
		...
		synchronized (journalCode.intern()) {
			ContentAssembly yearCa = ProjectBrowserUtils.getChildCaByNameAndType(context, journalArchvie,
					JOURNAL_ARCHIVE_YEAR, year);
			if (yearCa == null) {
				yearCa = ProjectContentAssemblyUtils.createContentAssembly(context, journalArchvie.getId(), year,
						JOURNAL_ARCHIVE_YEAR);
			}

			monthCa = ProjectBrowserUtils.getChildCaByNameAndType(context, yearCa, JOURNAL_ARCHIVE_MONTH, month);
			if (monthCa == null) {
				monthCa = ProjectContentAssemblyUtils.createContentAssembly(context, yearCa.getId(), month,
						JOURNAL_ARCHIVE_MONTH);
			}
		}
	...
	public ContentAssembly getArchiveCa() throws RSuiteException {
		ContentAssembly journalArchvie = ProjectBrowserUtils.getChildCaByType(context, journalCa, JOURNAL_ARCHIVE);
	...
	public static List<ManagedObject> getAvailableArticles(ExecutionContext context, User user, String journalCode)
			throws RSuiteException {
		SearchService srchSvc = context.getSearchService();
		String articleQuery = "/rs_ca_map/rs_ca[rmd:get-type(.) = 'article' and "
				+ "rmd:get-lmd-value(., 'journal_code') = '" + journalCode + "' and " + "rmd:get-lmd-value(., '"
				+ LMD_FIELD_ARTICLE_AVAILABLE + "') = '" + LMD_VALUE_YES + "']";

		articleQuery = XpathUtils.resolveRSuiteFunctionsInXPath(articleQuery);

		List<ManagedObject> articleSet = srchSvc.executeXPathSearch(user, articleQuery, 1, 0);
		return articleSet;
	}
	...
	public String getAbbreviatedTitle() throws RSuiteException {
		String title = journalCa.getLayeredMetadataValue(LMD_FIELD_JOURNAL_ABBRV_TITLE);
	...
	public boolean isAutomaticPdfGenerationWorkflow() throws RSuiteException {
		...
		if (StringUtils.isNotBlank(generationType)
				&& JournalGenerationType.AUTOMATED == JournalGenerationType.valueOf(generationType)) {
			return true;
		}
	...
	public JournalArticles getArticles() throws RSuiteException {
		ContentAssembly articlesCA = ProjectBrowserUtils.getChildCaByType(context, journalCa,
				JournalConstants.CA_TYPE_ARTICLES);
	...
	private User getUserWithValidation(String lmdName, String userType) throws RSuiteException {
		String userId = journalCa.getLayeredMetadataValue(lmdName);

		if (StringUtils.isBlank(userId)) {
			throw new RSuiteException(userType + " is not defined for the journal");
		}
