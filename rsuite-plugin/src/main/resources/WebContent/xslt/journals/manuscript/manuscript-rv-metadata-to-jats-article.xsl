...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	xmlns:xs="http://www.w3.org/2001/XMLSchema" exclude-result-prefixes="xs iet xlink"
	version="2.0">
	<xsl:output
	...
	<xsl:template name="create-permissions-element-for-open-access">
		<xsl:variable name="type"
			select="lower-case(replace($licenseType, 'CC-', ''))" />

		<xsl:variable name="licenseTypeName">
			<xsl:choose>
				<xsl:when test="$licenseType = 'CC-BY'" />
				<xsl:when test="$licenseType = 'CC-BY-NC'">
					-NonCommercial
				</xsl:when>
				<xsl:when test="$licenseType = 'CC-BY-ND'">
					-NoDerivs
				</xsl:when>
				<xsl:when test="$licenseType = 'CC-BY-NC-ND'">
		...
		<license license-type="open-access"
			...
			<license-p>
				...
				Creative Commons
				Attribution
				<xsl:value-of select="$licenseTypeName" />
				License (
				<ext-link xlink:href="http://creativecommons.org/licenses/{$type}/3.0/">
	...
	<xsl:template name="create-permissions-element-for-not-open-access">
		<xsl:choose>
			<xsl:when test="$licenseType = 'IET'">
				<copyright-statement>© Institution of Engineering and Technology
				</copyright-statement>
			</xsl:when>
