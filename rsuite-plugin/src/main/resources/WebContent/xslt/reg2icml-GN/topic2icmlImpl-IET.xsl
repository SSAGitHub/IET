...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template name="makeInCopyArticle">
    ...
    <Document DOMVersion="7.5" Self="d">
      ...
      <RootCellStyleGroup Self="u4ffc">
        ...
        <CellStyle Self="CellStyle/orangeBoxCell"
          AppliedParagraphStyle="ParagraphStyle/$ID/[No paragraph style]" Name="orangeBoxCell">
          <Properties>
            <BasedOn type="string">$ID/[None]</BasedOn>
          </Properties>
        </CellStyle>
      ...
      <RootTableStyleGroup Self="u4ff9">
        ...
        <TableStyle Self="TableStyle/orangeBox" Name="orangeBox">
          <Properties>
            <BasedOn type="string">$ID/[No table style]</BasedOn>
          </Properties>
        </TableStyle>
      <TableStyle Self="TableStyle/Sides" Name="Sides">
          <Properties>
            <BasedOn type="string">$ID/[No table style]</BasedOn>
          </Properties>
        </TableStyle>
  ...
  <xsl:template
    match="*[df:class(., 'chapter/chapterzzz')]/*[df:class(., 'topic/title')]
    /*[df:class(., 'd4p_simplenum-d/d4pSimpleEnumerator')]
    "
    mode="block-children" priority="20"> </xsl:template>

  <xsl:template
    match="*[df:class(., 'division/divisionzz')]/*[df:class(., 'topic/title')]
    /*[df:class(., 'd4p_simplenum-d/d4pSimpleEnumerator')]
    "
    mode="block-children" priority="20"/>
  ...
  <xsl:template match="*[df:class(., ' hi-d/b ')]" priority="100">
    <!-- Correctly handle paragraphs that contain mixed content with block-creating elements.
      -->
    <xsl:param name="articleType" as="xs:string" tunnel="yes"/>
    <xsl:variable name="pStyle" select="e2s:getPStyleForElement(., $articleType)" as="xs:string"/>
    <xsl:variable name="cStyle" select="e2s:getCStyleForElement(.)" as="xs:string"/>
    <xsl:message>BOLD1:pStyle: <xsl:value-of select="$pStyle"/></xsl:message>
    <xsl:message>BOLD2:cStyle: <xsl:value-of select="$cStyle"/></xsl:message>
    <xsl:for-each-group select="* | text()"
      group-adjacent="if (self::*) then if (df:isBlock(.)) then 'block' else 'text' else 'text'">
      <xsl:choose>
        <xsl:when test="self::* and df:isBlock(.)">
...
-->
  ...
  <xsl:template match="*[df:class(., 'topic/bodydiv')][@outputclass ='box']" priority="20">
    <xsl:param name="articleType" as="xs:string" tunnel="yes"/>
    <xsl:variable name="boxSequence"
      select="count(preceding::*[df:class(., 'topic/bodydiv')][@outputclass='box'])"/>
    <xsl:variable name="tempTable">
      <table class="- topic/table " frame="all" id="box_{$boxSequence}" outputclass="orangeBox"
        Justification="CenterJustified">
        <tgroup class="- topic/tgroup " cols="1" outputclass="orangeBox">
          <colspec class="- topic/colspec " colname="c1" colnum="1" colwidth="365pt"/>
          <tbody class="- topic/tbody ">
            <row class="- topic/row">
              <entry class="- topic/entry " valign="top" outputclass="box">
                <xsl:copy-of select="./*"/>
              </entry>
            </row>
          </tbody>
    ...
    <!--     <xsl:variable name="outPath"
      select="concat('file:///C:/temp/IET/Box/orangeBox', $boxSequence, '.xml')"/>
-->
  ...
  <xsl:template match="*[df:class(., 'topic/note')]" priority="10">
    ...
    <xsl:if test="$plural &gt; 1 and @outputclass != 'note_header'">
      <ParagraphStyleRange AppliedParagraphStyle="ParagraphStyle/{$pStyle}">
        <CharacterStyleRange AppliedCharacterStyle="CharacterStyle/$ID/[No character style]">
          <Br/>
        </CharacterStyleRange>
...
-->
          <xsl:choose>
            ...
            <xsl:when test="parent::*[df:class(., 'topic/ul')][@outputclass='triangle']">
              <xsl:choose>
                <xsl:when test="$listCount=1"> </xsl:when>
                <xsl:otherwise>
                  <xsl:text/>
                  <xsl:text/>
                </xsl:otherwise>
  ...
  <xsl:template
    ...
    *[df:class(., 'topic/p')]
    ...
    <xsl:variable name="cStyle" select="e2s:getCStyleForElement(.)" as="xs:string"/>
    <xsl:if
      test="preceding-sibling::*[df:class(., 'topic/ul')] and parent::*[df:class(., 'topic/li')]">
      <ParagraphStyleRange AppliedParagraphStyle="ParagraphStyle/{$pStyle}">
        <CharacterStyleRange AppliedCharacterStyle="CharacterStyle/$ID/[No character style]">
          <Content>&#x200B;&#x09;&#x200B;</Content>
        </CharacterStyleRange>
  ...
  <xsl:template name="OutputTextframezz">
  ...
  <xsl:template name="OutputTextframe">
    ...
    <xsl:choose>
      <xsl:when test="$citation_count=1">
        ...
        <TextFrame Self="u181db" ParentStory="u181c7" PreviousTextFrame="n" NextTextFrame="n"
            ...
            <PathGeometry>
              <GeometryPathType PathOpen="false">
                <PathPointArray>
                  ...
                  <PathPointType Anchor="80.78709448818896 3.0830435685285815"
                    LeftDirection="80.78709448818896 3.0830435685285815"
                    RightDirection="80.78709448818896 3.0830435685285815"/>
                  <PathPointType Anchor="80.78709448818896 -6" LeftDirection="80.78709448818896 -6"
                    RightDirection="80.78709448818896 -6"/>
                </PathPointArray>
              </GeometryPathType>
            </PathGeometry>
          </Properties>
          <TextFramePreference TextColumnFixedWidth="93.543" TextColumnMaxWidth="0"
          ...
          <AnchoredObjectSetting AnchoredPosition="Anchored" AnchorPoint="TopRightAnchor"
            ...
            AnchorXoffset="8.503937007874017"/>
          ...
          <xsl:call-template name="citationText">
            <xsl:with-param name="citationMargins">
              <xsl:sequence select="$citations"/>
            </xsl:with-param>
          </xsl:call-template>
        </TextFrame>
      ...
      <xsl:when test="$citation_count=2">
        ...
        <TextFrame Self="u59e" ParentStory="u58a" PreviousTextFrame="n" NextTextFrame="n"
          ContentType="TextType" ParentInterfaceChangeCount="138508 692660859 138558 11953775"
          TargetInterfaceChangeCount="" LastUpdatedInterfaceChangeCount=""
          ...
          GradientStrokeHiliteLength="0" GradientStrokeHiliteAngle="0"
          AppliedObjectStyle="ObjectStyle/$ID/[None]" Visible="true" Name="$ID/"
          ItemTransform="1 0 0 1 30.5 -2">
          <Properties>
            <PathGeometry>
              <GeometryPathType PathOpen="false">
                <PathPointArray>
                  <PathPointType Anchor="-30.5 -22" LeftDirection="-30.5 -22"
                    RightDirection="-30.5 -22"/>
                  <PathPointType Anchor="-30.5 2" LeftDirection="-30.5 2" RightDirection="-30.5 2"/>
                  <PathPointType Anchor="63.04330708679521 2" LeftDirection="63.04330708679521 2"
                    RightDirection="63.04330708679521 2"/>
                  <PathPointType Anchor="63.04330708679521 -22"
                    LeftDirection="63.04330708679521 -22" RightDirection="63.04330708679521 -22"/>
                </PathPointArray>
              </GeometryPathType>
            </PathGeometry>
          </Properties>
          <TextFramePreference TextColumnFixedWidth="93.54330708679521" TextColumnMaxWidth="0"
            AutoSizingType="Off" AutoSizingReferencePoint="CenterPoint"
            UseMinimumHeightForAutoSizing="false" MinimumHeightForAutoSizing="0"
            UseMinimumWidthForAutoSizing="false" MinimumWidthForAutoSizing="0"
            UseNoLineBreaksForAutoSizing="false"/>
          <AnchoredObjectSetting AnchoredPosition="Anchored" PinPosition="false"
            AnchorPoint="TopRightAnchor" VerticalAlignment="TopAlign"
      ...
      <xsl:when test="$citation_count=3">
        ...
        <TextFrame Self="u1914" ParentStory="u1900" PreviousTextFrame="n" NextTextFrame="n"
          ContentType="TextType" ParentInterfaceChangeCount="" TargetInterfaceChangeCount=""
          LastUpdatedInterfaceChangeCount="" OverriddenPageItemProps=""
          HorizontalLayoutConstraints="FlexibleDimension FixedDimension FlexibleDimension"
          VerticalLayoutConstraints="FlexibleDimension FixedDimension FlexibleDimension"
          ...
          GradientStrokeHiliteLength="0" GradientStrokeHiliteAngle="0"
          AppliedObjectStyle="ObjectStyle/$ID/[None]" Visible="true" Name="$ID/"
          ItemTransform="1 0 0 1 36.00000000000001 -2.2677165354333937">
          <Properties>
            <PathGeometry>
              <GeometryPathType PathOpen="false">
                <PathPointArray>
                  <PathPointType Anchor="-36.00000000000001 -36"
                    LeftDirection="-36.00000000000001 -36" RightDirection="-36.00000000000001 -36"/>
                  <PathPointType Anchor="-36.00000000000001 2.2677165354333972"
                    LeftDirection="-36.00000000000001 2.2677165354333972"
                    RightDirection="-36.00000000000001 2.2677165354333972"/>
                  <PathPointType Anchor="57.54330708661416 2.2677165354333972"
                    LeftDirection="57.54330708661416 2.2677165354333972"
                    RightDirection="57.54330708661416 2.2677165354333972"/>
                  <PathPointType Anchor="57.54330708661416 -36"
                    LeftDirection="57.54330708661416 -36" RightDirection="57.54330708661416 -36"/>
                </PathPointArray>
              </GeometryPathType>
            </PathGeometry>
          </Properties>
          <TextFramePreference TextColumnFixedWidth="93.54330708661418" TextColumnMaxWidth="0"
            AutoSizingType="Off" AutoSizingReferencePoint="CenterPoint"
            UseMinimumHeightForAutoSizing="false" MinimumHeightForAutoSizing="0"
            UseMinimumWidthForAutoSizing="false" MinimumWidthForAutoSizing="0"
            UseNoLineBreaksForAutoSizing="false"/>
      ...
      <xsl:otherwise>
        ...
        <TextFrame Self="u1be79" ParentStory="u1be65" PreviousTextFrame="n" NextTextFrame="n"
          ContentType="TextType" ParentInterfaceChangeCount="" TargetInterfaceChangeCount=""
          LastUpdatedInterfaceChangeCount="" OverriddenPageItemProps=""
          HorizontalLayoutConstraints="FlexibleDimension FixedDimension FlexibleDimension"
          VerticalLayoutConstraints="FlexibleDimension FixedDimension FlexibleDimension"
          ...
          GradientStrokeHiliteLength="0" GradientStrokeHiliteAngle="0"
          AppliedObjectStyle="ObjectStyle/$ID/[None]" Visible="true" Name="$ID/"
          ItemTransform="1 0 0 1 36.00000000000001 -13.737323082600824">
          <Properties>
            <PathGeometry>
              <GeometryPathType PathOpen="false">
                <PathPointArray>
                  <PathPointType Anchor="-36.00000000000001 -36"
                    LeftDirection="-36.00000000000001 -36" RightDirection="-36.00000000000001 -36"/>
                  <PathPointType Anchor="-36.00000000000001 13.737323082600827"
                    LeftDirection="-36.00000000000001 13.737323082600827"
                    RightDirection="-36.00000000000001 13.737323082600827"/>
                  <PathPointType Anchor="57.54330708661416 13.737323082600827"
                    LeftDirection="57.54330708661416 13.737323082600827"
                    RightDirection="57.54330708661416 13.737323082600827"/>
                  <PathPointType Anchor="57.54330708661416 -36"
                    LeftDirection="57.54330708661416 -36" RightDirection="57.54330708661416 -36"/>
                </PathPointArray>
              </GeometryPathType>
  ...
  <xsl:template name="citationText">
    <xsl:param name="citationMargins"/>
    <xsl:for-each select="$citationMargins/cite_margin">
      <xsl:variable name="pStyle">
        <xsl:choose>
          <xsl:when test="./descendant::i">
            <xsl:text>Reg nos Italic</xsl:text>
          </xsl:when>
        <xsl:otherwise>
            <xsl:text>Reg nos</xsl:text> 
          </xsl:otherwise>
        </xsl:choose>
      </xsl:variable>
      <ParagraphStyleRange AppliedParagraphStyle="ParagraphStyle/{$pStyle}">
        <CharacterStyleRange AppliedCharacterStyle="CharacterStyle/Regulation nos">
          <Content>
        ...
        <PathGeometry>
          <GeometryPathType PathOpen="false">
            <PathPointArray>
              ...
              <PathPointType Anchor="-6 16.525106544511857" LeftDirection="-6 16.525106544511857"
                RightDirection="-6 16.525106544511857"/>
              <PathPointType Anchor="18.953605926273426 16.525106544511857"
                LeftDirection="18.953605926273426 16.525106544511857"
                RightDirection="18.953605926273426 16.525106544511857"/>
              <PathPointType Anchor="18.953605926273426 -6" LeftDirection="18.953605926273426 -6"
                RightDirection="18.953605926273426 -6"/>
            </PathPointArray>
          </GeometryPathType>
        </PathGeometry>
      </Properties>
...
<?xpacket end="r"?>]]></Contents>
        ...
        <Link Self="u10cd8" AssetURL="$ID/" AssetID="$ID/" LinkResourceURI="{$linkUri2}"
          LinkResourceFormat="$ID/{$extension}" StoredState="Normal" LinkClassID="35906"
          LinkClientID="257" LinkResourceModified="false" LinkObjectModified="false" ShowInUI="true"
          CanEmbed="true" CanUnembed="true" CanPackage="true" ImportPolicy="NoAutoImport"
...
<?xpacket end="r"?>]]></Contents>
  ...
  <xsl:template match="*[df:class(., 'topic/image')]">
    ...
    <Rectangle Self="{generate-id()}" FillColor="Color/Paper">
      <Properties>
        ...
        <PathGeometry>
          <GeometryPathType PathOpen="false">
            <PathPointArray>
              ...
              <PathPointType Anchor="286.99999999981907 260.559055128042"
                LeftDirection="286.99999999981907 260.559055128042"
                RightDirection="286.99999999981907 260.559055128042"/>
              <PathPointType Anchor="286.99999999981907 -47" LeftDirection="286.99999999981907 -47"
                RightDirection="286.99999999981907 -47"/>
            </PathPointArray>
          </GeometryPathType>
        </PathGeometry>
      </Properties>
      <!-- Proportional fitting -->
      <FrameFittingOption AutoFit="false" LeftCrop="0" TopCrop="-87.39155923273314"
        RightCrop="22.365016263100983" BottomCrop="143.23151786205875"
        FittingOnEmptyFrame="Proportionally" FittingAlignment="CenterAnchor"/>
      <Image ImageRenderingIntent="UseColorSettings" AppliedObjectStyle="ObjectStyle/$ID/[None]"
        Visible="true" Name="$ID/" Self="rc_{concat(generate-id(),'Image')}">
        <Link Self="{concat(generate-id(),'Link')}" AssetURL="$ID/" AssetID="$ID/"
          LinkResourceURI="{$linkUri2}" LinkResourceFormat="$ID/{$extension}"/>
      </Image>
    </Rectangle>
