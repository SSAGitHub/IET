...
-->
  ...
  <xsl:template match="*[df:class(., 'topic/tgroup')]">
    ...
    <xsl:if test="$numCols != count(*[df:class(., 'topic/colspec')])">
      ...
      <xsl:message> + [WARN] Maximum column count (<xsl:value-of select="$numCols"/>) not equal to
        number of colspec elements (<xsl:value-of select="count(*[df:class(., 'topic/colspec')])"
        />).</xsl:message>
    </xsl:if>
    <Table AppliedTableStyle="TableStyle/$ID/{$tStyle}" TableDirection="LeftToRightDirection"
  ...
  <xsl:template match="*[df:class(., 'topic/entry')]">
    ...
    <Cell Name="{$colNumber}:{$rowNumber}" RowSpan="{$rowSpan}" ColumnSpan="{$colSpan}"
      AppliedCellStyle="CellStyle/$ID/${cellStyle}" ppcs="l_0" Self="rc_{generate-id()}">
       <xsl:attribute name="LeftEdgeStrokeWeight">0.75</xsl:attribute>
      <xsl:attribute name="RightEdgeStrokeWeight">0.75</xsl:attribute>
      <xsl:attribute name="TopEdgeStrokeWeight">0.75</xsl:attribute>
      <xsl:attribute name="BottomEdgeStrokeWeight">0.75</xsl:attribute>
      <xsl:if test="($colNumber = 0) and ancestor::*[df:class(.,'topic/table')][@frame='topbot']">
        <xsl:attribute name="LeftEdgeStrokeWeight">0</xsl:attribute>
      </xsl:if>
      <xsl:if test="($colNumber + $colSpan = $colCount) and ancestor::*[df:class(.,'topic/table')][@frame='topbot']">
        <xsl:attribute name="RightEdgeStrokeWeight">0</xsl:attribute>
      </xsl:if>
      <xsl:if test="($rowNumber = 0) and ancestor::*[df:class(.,'topic/table')][@frame='topbot']">
        <xsl:attribute name="TopEdgeStrokeWeight">1.5</xsl:attribute>
      </xsl:if>
      <xsl:if test="($rowNumber + $rowSpan = $rowCount) and ancestor::*[df:class(.,'topic/table')][@frame='topbot']">
        <xsl:attribute name="BottomEdgeStrokeWeight">1.5</xsl:attribute>
      </xsl:if>
      <xsl:if test="ancestor::*[df:class(.,'topic/table')][@frame='none']">
        <xsl:attribute name="LeftEdgeStrokeWeight">0</xsl:attribute>
        <xsl:attribute name="RightEdgeStrokeWeight">0</xsl:attribute>
        <xsl:attribute name="TopEdgeStrokeWeight">0</xsl:attribute>
        <xsl:attribute name="BottomEdgeStrokeWeight">0</xsl:attribute>
      </xsl:if>
      <xsl:if test="@outputclass='index'">
