...
<xsl:import href="../../net.sourceforge.dita4publishers.common.xslt/xsl/lib/dita-support-lib.xsl"/>
  ...
  <!-- ONSITE GUIDE STYLES BELOW HERE -->
  
  <xsl:template match="*[df:class(., 'index/index')]/*[df:class(., 'topic/title')]"
    mode="style-map-pstyle" priority="100">
    <xsl:sequence select="'chaptitle'"/>
  </xsl:template>
  ...
  <xsl:template match="*[df:class(., 'chapter/chapter')]/*[df:class(., 'topic/title')]/*[df:class(., 'd4p_simplenum-d/d4pSimpleEnumerator')]" 
    mode="style-map-pstyle" priority="20">
    <xsl:sequence select="'chapno'"/>
  </xsl:template>

  <xsl:template match="*[df:class(., 'division/division')]/*[df:class(., 'topic/title')]"
    mode="style-map-pstyle" priority="5">
    <xsl:sequence select="'A head'"/>
  </xsl:template>
  
  <xsl:template match="*[df:class(., 'division/division')]/*[df:class(., 'division/division')]/*[df:class(., 'topic/title')]" 
    mode="style-map-pstyle" priority="20">
    <xsl:sequence select="'B Head'"/>
  </xsl:template>
  
  <xsl:template
    match="*[df:class(., 'division/division')]/*[df:class(., 'division/division')]/*[df:class(., 'division/division')]/*[df:class(., 'topic/title')]"
  ...
  <xsl:template
    match="*[df:class(., 'chapter/chapter')]//*[df:class(., 'topic/ol')]/*[df:class(., 'topic/li')]/*[df:class(., 'topic/p')]"
    mode="style-map-pstyle" priority="25">
    ...
    <xsl:choose>
      <xsl:when test="$listCount=1">
        <xsl:choose>
          <xsl:when test="count(parent::*[df:class(., 'topic/li')]/following-sibling::*[df:class(., 'topic/li')]) > 0">
            <xsl:sequence select="'Body text list'"/>
          </xsl:when>
          <xsl:otherwise>
            <xsl:sequence select="'Body text list last'"/>
          </xsl:otherwise>
        </xsl:choose>
      </xsl:when>
      <xsl:otherwise>
  ...
  <xsl:template
    match="*[df:class(., 'topic/ol')]/*[df:class(., 'topic/li')]"
    mode="style-map-pstyle" priority="20">
    <xsl:variable name="listCount" select="count(ancestor::*[df:class(., 'topic/ol')]) + count(ancestor::*[df:class(., 'topic/ul')])"/>
    <xsl:choose>
      <xsl:when test="$listCount=1">
        <xsl:choose>
          <xsl:when test="count(following-sibling::*[df:class(., 'topic/li')]) > 0">
            <xsl:sequence select="'Body text list'"/>
          </xsl:when>
          <xsl:otherwise>
            <xsl:sequence select="'Body text list last'"/>
      ...
      <xsl:otherwise>
        <xsl:sequence select="'Body text list 2'"/>
      </xsl:otherwise>
    </xsl:choose>
  </xsl:template>
  ...
  <xsl:template
    ...
    <xsl:choose>
      <xsl:when test="$listCount=1">
        <xsl:choose>
          <xsl:when test="count(following-sibling::*[df:class(., 'topic/li')]) > 0">
            <xsl:sequence select="'Body Text bullets'"/>
          </xsl:when>
          <xsl:otherwise>
            <xsl:sequence select="'Body Text bullets last'"/>
          </xsl:otherwise>
        </xsl:choose>
      </xsl:when>
      <xsl:otherwise>
  ...
  <xsl:template
    match="*[df:class(., 'topic/fig')]/*[df:class(., 'topic/note')]"
    mode="style-map-pstyle" priority="10">
    <xsl:sequence select="'Body Note'"/>
  </xsl:template>

  <xsl:template match="*[df:class(., 'chapter/chapter')]//*[df:class(., 'topic/fig')]/*[df:class(., 'topic/title')]" 
    mode="style-map-pstyle" priority="10">
    <xsl:sequence select="'Figure caption'"/>
  </xsl:template>
...
<!--
 <xsl:template match="/*[df:class(., 'topic/topic')]/*[df:class(., 'topic/title')]" 
  ...
  <xsl:template match="*[df:class(., 'topic/note')]" 
    mode="style-map-pstyle">
    <xsl:sequence select="'Body Note Text'"/>
  </xsl:template>

  <xsl:template match="*[df:class(., 'topic/note')]/*[df:class(., 'topic/ol')]/*[df:class(., 'topic/li')]" mode="style-map-pstyle" priority="30">
    <xsl:sequence select="'Body note list'"/>
  </xsl:template>

  <xsl:template match="*[df:class(., 'topic/ul')][ancestor::*[df:class(., 'topic/note')]]/*[df:class(., 'topic/li')]" mode="style-map-pstyle" priority="30">
    <xsl:sequence select="'Body note bullet'"/>
  </xsl:template>
  
  <xsl:template
    match="*[df:class(., 'topic/ul')][@outputclass='arabic']/*[df:class(., 'topic/li')]"
    mode="style-map-pstyle" priority="35">
    <xsl:sequence select="'Body Text numbers'"/>
  </xsl:template>
  
  <xsl:template
    match="*[df:class(., 'topic/ul')][@outputclass='asterix'][ancestor::*[df:class(., 'topic/note')]]/*[df:class(., 'topic/li')]"
    mode="style-map-pstyle" priority="35">
    <xsl:sequence select="'Body note'"/>
  </xsl:template>

  <xsl:template
    match="*[df:class(., 'topic/ul')][@outputclass='dagger'][ancestor::*[df:class(., 'topic/note')]]/*[df:class(., 'topic/li')]"
    mode="style-map-pstyle" priority="35">
    <xsl:sequence select="'Body note'"/>
  </xsl:template>
  ...
  <xsl:template
    ...
    <xsl:sequence select="'Body Text ul'"/>
  </xsl:template>
...
<!--  <xsl:template 
  ...
  <xsl:template
    ...
    <xsl:choose>
      ...
      <xsl:otherwise>
        <xsl:sequence select="'Body text bullets 2'"/>
      </xsl:otherwise>
    </xsl:choose>
  </xsl:template>
  ...
  <xsl:template
    ...
    <xsl:choose>
      <xsl:when test="$listCount=1">
        <xsl:sequence select="'Body list 1 en rule'"/>
      </xsl:when>
      <xsl:otherwise>
        <xsl:sequence select="'Body list 2 en rule'"/>
      </xsl:otherwise>
    </xsl:choose>
  </xsl:template>
  ...
  <xsl:template match="*[df:class(., 'topic/table')]/*[df:class(., 'topic/title')]"
    mode="style-map-pstyle">
    <xsl:sequence select="'caption for table head long bold bit'"/>
  </xsl:template>
  ...
  <xsl:template
    match="*[df:class(., 'topic/thead')][ancestor::*[df:class(., 'topic/table')]/@frame='none']/*[df:class(., 'topic/row')]/*[df:class(., 'topic/entry')]"
    mode="style-map-pstyle" priority="20">
    <xsl:sequence select="'Table text heading-black'"/>
  </xsl:template>
  <xsl:template match="*[df:class(., 'topic/thead')]/*[df:class(., 'topic/row')]/*[df:class(., 'topic/entry')]" 
    mode="style-map-pstyle" priority="15">
    <xsl:sequence select="'Table text heading'"/>
  </xsl:template>
  
  <xsl:template match="*[df:class(., 'topic/section')]/*[df:class(., 'topic/title')]" 
    mode="style-map-pstyle">
    <!-- FIXME: account for outputclass -->
    <xsl:sequence select="'Index Letter'"/>
    </xsl:template>
...
<xsl:template match="*[df:class(., 'topic/xref')][not(ancestor::*[df:class(., ' cite_margin-d/cite_margin ')])]" mode="style-map-cstyle" priority="10">
    <xsl:sequence select="'XREF'"/>
  </xsl:template>
  ...
  <xsl:template 
    ...
    *[df:class(., 'hi-d/b')]/*[df:class(., 'hi-d/sub')]" 
    ...
    <xsl:sequence select="'bold-subscript'"/>
  </xsl:template>
  ...
  <xsl:template 
    ...
    *[df:class(., 'hi-d/i')]/*[df:class(., 'hi-d/sub')]" 
    mode="style-map-cstyle" priority="15">
    <xsl:sequence select="'italic-subscript'"/>
  </xsl:template>
  ...
  <xsl:template 
    ...
    *[df:class(., 'hi-d/b')]/*[df:class(., 'hi-d/sup')]" 
    mode="style-map-cstyle" priority="15">
    <xsl:sequence select="'bold-superscript'"/>
  </xsl:template>
  ...
  <xsl:template 
    ...
    *[df:class(., 'hi-d/i')]/*[df:class(., 'hi-d/sup')]" 
    mode="style-map-cstyle" priority="15">
    <xsl:sequence select="'italic-superscript'"/>
  </xsl:template>
