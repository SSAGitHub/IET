...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="*[df:class(., 'index/index')]/*[df:class(., 'topic/title')]" mode="#all"/>

  <xsl:template 
    match="
    *[df:class(., 'hi-d/i')][not(ancestor::*[df:class(., 'topic/title')])][*[df:class(., ' regnum-d/regnum')]]" 
    mode="#all">
    ...
    <xsl:for-each-group select="* | text()"
      group-adjacent="if (self::*) then if (df:isBlock_regnum(.)) then 'block' else 'text' else 'text'">
      <xsl:choose>
        <xsl:when test="self::* and df:isBlock_regnum(.)">
          <xsl:apply-templates select="current-group()" mode="block-children"/>
        </xsl:when>
        <xsl:otherwise>
  ...
  <xsl:template 
    match="*[df:class(., 'hi-d/i')][parent::*[df:class(., 'topic/title')]][*[df:class(., ' regnum-d/regnum')]]" 
    mode="#all">
    <!-- The block-children mode handles text and elements that are (effectively) direct
      children of block elements, meaning that they have no ancestor inline elements
      from which then need to inherit style info.
  ...
  <!-- SA00 attempt to output bold with childeren, but never being invoked -->
  <xsl:template match="*[df:class(., 'hi-d/izzzz')]" priority="100">
    <!-- Correctly handle paragraphs that contain mixed content with block-creating elements.
      -->
    <xsl:param name="articleType" as="xs:string" tunnel="yes"/>
    <xsl:variable name="pStyle" select="e2s:getPStyleForElement(., $articleType)" as="xs:string"/>
    <xsl:variable name="cStyle" select="e2s:getCStyleForElement(.)" as="xs:string"/>
    <!--   <xsl:message>Italic1:pStyle: <xsl:value-of select="$pStyle"/></xsl:message>
    <xsl:message>Italic:cStyle: <xsl:value-of select="$cStyle"/></xsl:message>
-->
    <xsl:for-each-group select="* | text()"
  ...
  <xsl:template
    match="
    *[df:class(., 'topic/body')][//*[df:class(., 'topic/bodydiv')][@outputclass='symbol_information']]">
    <xsl:param name="articleType" as="xs:string" tunnel="yes"/>
    
    <xsl:apply-templates select="*[df:class(., 'topic/p')]"/>   
    <xsl:variable name="tempTable">
      <table class="- topic/table " frame="all" id="rtab41.1">
        <tgroup class="- topic/tgroup " cols="9">
          <colspec class="- topic/colspec " colname="c1" colnum="1" colwidth="80pt"/>
          <colspec class="- topic/colspec " colname="c2" colnum="2" colwidth="160pt"/>
          <colspec class="- topic/colspec " colname="c3" colnum="3" colwidth="50pt"/>
          <colspec class="- topic/colspec " colname="c4" colnum="4" colwidth="100pt"/>
          <tbody class="- topic/tbody ">
            <xsl:apply-templates mode="list2table"/>
          </tbody>
        </tgroup>
      </table>
    </xsl:variable>
  ...
  <xsl:template match="*[df:class(., 'topic/bodydiv')][@outputclass='symbol_information']"
    mode="list2table">
    <row class="- topic/row">
      <xsl:apply-templates mode="list2table"/>
    </row>
  ...
  <xsl:template match="*[df:class(., 'topic/p')]" mode="list2table">
  </xsl:template>

  <xsl:template match="*[df:class(., 'topic/dt')][@outputclass='symbol']" mode="list2table">
    <entry class="- topic/entry " outputclass="symbols">
      <xsl:value-of select="."/>
    </entry>
  </xsl:template>
  
  <xsl:template match="*[df:class(., 'topic/dd')]" mode="list2table">
    <entry class="- topic/entry " outputclass="symbols">
      <xsl:value-of select="."/>
    </entry>
  </xsl:template>

  <xsl:template match="*[df:class(., 'topic/p')][@outputclass='symbol_suffix']" mode="list2table">
    <entry class="- topic/entry " outputclass="symbols">
      <xsl:value-of select="."/>
    </entry>
  </xsl:template>

  <xsl:template match="*[df:class(., 'topic/xref')][@outputclass='first_occurrence']" mode="list2table">
    <xsl:if test="not(preceding-sibling::*[df:class(., 'topic/p')][@outputclass='symbol_suffix'])">
  ...
  <xsl:template match="*" mode="list2table" priority="-1">
    <xsl:apply-templates mode="list2table"/>
  </xsl:template>
  ...
  <xsl:template match="*[df:class(., 'topic/title')][*[df:class(., 'd4p-formatting-d/br')]]"
    priority="5">
    <xsl:param name="articleType" as="xs:string" tunnel="yes"/>
    <xsl:variable name="line1">
      <xsl:value-of
        select=".//text()[following::br][1]"/>
    </xsl:variable>
    <xsl:variable name="line2">
      <xsl:value-of select=".//text()[preceding::br][1]"/>
    </xsl:variable>
    <xsl:variable name="pStyle" select="e2s:getPStyleForElement(., $articleType)" as="xs:string"/>
    ...
    <xsl:call-template name="makeBlock-cont">
      <xsl:with-param name="pStyle" select="$pStyle" as="xs:string" tunnel="yes"/>
      <xsl:with-param name="cStyle" select="$cStyle" as="xs:string" tunnel="yes"/>
      <xsl:with-param name="content" as="node()*" select="$line1"/>
    </xsl:call-template>
    <xsl:call-template name="makeBlock-cont">
      <xsl:with-param name="pStyle" select="$pStyle" as="xs:string" tunnel="yes"/>
      <xsl:with-param name="cStyle" select="$cStyle" as="xs:string" tunnel="yes"/>
      <xsl:with-param name="content" as="node()*" select="$line2"/>
    </xsl:call-template>
  </xsl:template>
  <xsl:template match="
    *[df:class(., 'topic/note')][count(node())=1 and ph[@status='deleted']]" priority="20"/>
  ...
  <xsl:template match="*[df:class(., 'topic/note')]" priority="10">
    ...
    <xsl:variable name="pStyle" select="e2s:getPStyleForElement(., $articleType)" as="xs:string"/>
    <xsl:variable name="cStyle" select="e2s:getCStyleForElement(.)" as="xs:string"/>
    <xsl:variable name="precedingNote"
      select="count(preceding-sibling::*[df:class(., 'topic/note')][not(count(node())=1 and ph[@status='deleted'])]) + 
      count(parent::entry/parent::row/preceding-sibling::row/entry/note[not(count(node())=1 and ph[@status='deleted'])])"/>
    <xsl:variable name="followingNote"
      select="count(following-sibling::*[df:class(., 'topic/note')][not(count(node())=1 and ph[@status='deleted'])]) + 
      count(parent::entry/parent::row/following-sibling::row/entry/note[not(count(node())=1 and ph[@status='deleted'])])"/>
    <ParagraphStyleRange AppliedParagraphStyle="ParagraphStyle/{$pStyle}">
      <CharacterStyleRange AppliedCharacterStyle="CharacterStyle/StrongArial">
        <xsl:choose>
          <xsl:when test="$precedingNote = 0 and $followingNote > 0">
            <Content>NOTE 1:&#x09;</Content>
          </xsl:when>
          <xsl:when test="$precedingNote > 0">
            <Content>NOTE <xsl:value-of select="$precedingNote + 1"/>:&#x09;</Content>
          </xsl:when>
        <xsl:otherwise>
    ...
    <xsl:variable name="tempTable">
      <table class="- topic/table " frame="none" id="index">
        <tgroup class="- topic/tgroup " cols="3">
          <colspec class="- topic/colspec " colname="c1" colnum="1" colwidth="140pt"/>
          <colspec class="- topic/colspec " colname="c2" colnum="2" colwidth="1pt"/>
          <colspec class="- topic/colspec " colname="c3" colnum="3" colwidth="80pt"/>
          <tbody class="- topic/tbody ">
  ...
  <xsl:template match="
    *[df:class(., 'topic/li')][count(node())=1 and ph[@status='deleted']]" priority="20"/>
  ...
  <xsl:template match="
    *[df:class(., 'topic/li')]
    ...
    <!-- Correctly handle paragraphs that contain mixed content with block-creating elements.
      -->
    <xsl:param name="articleType" as="xs:string" tunnel="yes"/>
    <xsl:variable name="li-count" select="count(preceding-sibling::li[not(count(node())=1 and ph[@status='deleted'])])+1"/>
    <xsl:variable name="listCount" select="count(ancestor::*[df:class(., 'topic/ol')]) + count(ancestor::*[df:class(., 'topic/ul')])"/>
    <xsl:variable name="pStyle" select="e2s:getPStyleForElement(., $articleType)" as="xs:string"/>
    <xsl:variable name="cStyle1" select="e2s:getCStyleForElement(.)" as="xs:string"/>
    <xsl:variable name="cStyle">
      <xsl:choose>
        <xsl:when test="contains($cStyle1, '/')">
          <xsl:text>[No character style]</xsl:text>
          </xsl:when>
      <xsl:otherwise>
          <xsl:value-of select="$cStyle1"/>
        </xsl:otherwise>
      </xsl:choose>
    </xsl:variable>
    <ParagraphStyleRange AppliedParagraphStyle="ParagraphStyle/{$pStyle}">
      <CharacterStyleRange AppliedCharacterStyle="CharacterStyle/$ID/{$cStyle}">
        ...
        <xsl:choose>
            ...
            <xsl:when test="parent::*[df:class(., 'topic/ol')][@outputclass='letterLowerCaseRightParen']">
               <xsl:number value="$li-count" format="a"/>
              <xsl:text>)&#x09;</xsl:text>
            </xsl:when>
  ...
  <xsl:template match="*[df:class(., 'topic/fig')]">
    ...
    <xsl:apply-templates select="* except (*[df:class(., 'topic/title')])"/>
    </xsl:template>

  <xsl:template match="*[df:class(., 'topic/figgroup')]">
    <!-- Override this template to put the title before or after the 
         figure content.
      -->
    <xsl:apply-templates select="*[df:class(., 'topic/title')]"/>
  ...
  <xsl:template match="*[df:class(., 'topic/desc')]" priority="10">
    <xsl:param name="articleType" as="xs:string" tunnel="yes"/>
    <xsl:variable name="pStyle" select="e2s:getPStyleForElement(., $articleType)" as="xs:string"/>
    <xsl:variable name="cStyle" select="e2s:getCStyleForElement(.)" as="xs:string"/>
    <xsl:for-each-group select="* | text()"
  ...
  <xsl:template name="OutputTextframe">
    ...
    <xsl:choose>
      <xsl:when test="$citation_count=1">
        ...
        <TextFrame Self="ud066" ParentStory="ud052" PreviousTextFrame="n" NextTextFrame="n"
          ...
          ItemTransform="1 0 0 1 35.433070866141726 -3.0830435685285806">
          <Properties>
            <PathGeometry>
              <GeometryPathType PathOpen="false">
                <PathPointArray>
                  <PathPointType Anchor="-35.433070866141726 -6"
                    LeftDirection="-35.433070866141726 -6" RightDirection="-35.433070866141726 -6"/>
                  <PathPointType Anchor="-35.433070866141726 3.0830435685285815"
                    LeftDirection="-35.433070866141726 3.0830435685285815"
                    RightDirection="-35.433070866141726 3.0830435685285815"/>
          ...
          <TextFramePreference TextColumnFixedWidth="68.03149606299212" TextColumnMaxWidth="0"/>
          <AnchoredObjectSetting AnchoredPosition="Anchored" SpineRelative="false"
            LockPosition="false" PinPosition="true" AnchorPoint="TopRightAnchor"
            HorizontalAlignment="LeftAlign" HorizontalReferencePoint="TextFrame"
            VerticalAlignment="TopAlign" VerticalReferencePoint="Capheight" AnchorXoffset="0"
          ...
          <xsl:for-each select="$citations/cite_margin">
            <ParagraphStyleRange AppliedParagraphStyle="ParagraphStyle/Reg nos">
      ...
      <xsl:when test="$citation_count=2">
        ...
        <TextFrame Self="u1d274" ParentStory="u1d260" PreviousTextFrame="n" NextTextFrame="n" ContentType="TextType" ParentInterfaceChangeCount="" TargetInterfaceChangeCount="" LastUpdatedInterfaceChangeCount="" OverriddenPageItemProps="" HorizontalLayoutConstraints="FlexibleDimension FixedDimension FlexibleDimension" VerticalLayoutConstraints="FlexibleDimension FixedDimension FlexibleDimension" GradientFillStart="0 0" GradientFillLength="0" GradientFillAngle="0" GradientStrokeStart="0 0" GradientStrokeLength="0" GradientStrokeAngle="0" Locked="false" LocalDisplaySetting="Default" GradientFillHiliteLength="0" GradientFillHiliteAngle="0" GradientStrokeHiliteLength="0" GradientStrokeHiliteAngle="0" AppliedObjectStyle="ObjectStyle/$ID/[Normal Text Frame]" Visible="true" Name="$ID/" ItemTransform="1 0 0 1 36 14.551602100762747">
          <Properties>
            <PathGeometry>
              <GeometryPathType PathOpen="false">
                <PathPointArray>
                  <PathPointType Anchor="-36 -36" LeftDirection="-36 -36" RightDirection="-36 -36" />
                  <PathPointType Anchor="-36 -14.551602100762747" LeftDirection="-36 -14.551602100762747" RightDirection="-36 -14.551602100762747" />
                  <PathPointType Anchor="32.03149606299213 -14.551602100762747" LeftDirection="32.03149606299213 -14.551602100762747" RightDirection="32.03149606299213 -14.551602100762747" />
          ...
          <AnchoredObjectSetting AnchoredPosition="Anchored" SpineRelative="false" LockPosition="false" PinPosition="true" AnchorPoint="TopRightAnchor" HorizontalAlignment="LeftAlign" HorizontalReferencePoint="TextFrame" VerticalAlignment="TopAlign" VerticalReferencePoint="Capheight" AnchorXoffset="0" AnchorYoffset="0" AnchorSpaceAbove="0" />
      ...
      <xsl:when test="$citation_count=3">
        ...
        <TextFrame Self="u1d2eb" ParentStory="u1d2d7" PreviousTextFrame="n" NextTextFrame="n" ContentType="TextType" ParentInterfaceChangeCount="" TargetInterfaceChangeCount="" LastUpdatedInterfaceChangeCount="" OverriddenPageItemProps="" HorizontalLayoutConstraints="FlexibleDimension FixedDimension FlexibleDimension" VerticalLayoutConstraints="FlexibleDimension FixedDimension FlexibleDimension" StrokeWeight="1" GradientFillStart="0 0" GradientFillLength="0" GradientFillAngle="0" GradientStrokeStart="0 0" GradientStrokeLength="0" GradientStrokeAngle="0" Locked="false" LocalDisplaySetting="Default" GradientFillHiliteLength="0" GradientFillHiliteAngle="0" GradientStrokeHiliteLength="0" GradientStrokeHiliteAngle="0" AppliedObjectStyle="ObjectStyle/$ID/[None]" Visible="true" Name="$ID/" ItemTransform="1 0 0 1 36 3.6854603692430885">
          <Properties>
            <PathGeometry>
              <GeometryPathType PathOpen="false">
                <PathPointArray>
                  <PathPointType Anchor="-36 -36" LeftDirection="-36 -36" RightDirection="-36 -36" />
                  <PathPointType Anchor="-36 -3.685460369243089" LeftDirection="-36 -3.685460369243089" RightDirection="-36 -3.685460369243089" />
                  <PathPointType Anchor="32.03149606299213 -3.685460369243089" LeftDirection="32.03149606299213 -3.685460369243089" RightDirection="32.03149606299213 -3.685460369243089" />
                  <PathPointType Anchor="32.03149606299213 -36" LeftDirection="32.03149606299213 -36" RightDirection="32.03149606299213 -36" />
                </PathPointArray>
              </GeometryPathType>
            </PathGeometry>
          ...
          <TextFramePreference TextColumnFixedWidth="68.03149606299213" TextColumnMaxWidth="0" />
          <AnchoredObjectSetting AnchoredPosition="Anchored" AnchorPoint="TopRightAnchor" VerticalAlignment="TopAlign" VerticalReferencePoint="Capheight" />
          <TextWrapPreference Inverse="false" ApplyToMasterPageOnly="false" TextWrapSide="BothSides" TextWrapMode="None">
            <Properties>
              <TextWrapOffset Top="0" Left="0" Bottom="0" Right="0" />
            </Properties>
      ...
      <xsl:otherwise>
        ...
        <TextFrame Self="u1d57f" ParentStory="u1d56b" PreviousTextFrame="n" NextTextFrame="n" ContentType="TextType" ParentInterfaceChangeCount="" TargetInterfaceChangeCount="" LastUpdatedInterfaceChangeCount="" OverriddenPageItemProps="" HorizontalLayoutConstraints="FlexibleDimension FixedDimension FlexibleDimension" VerticalLayoutConstraints="FlexibleDimension FixedDimension FlexibleDimension" StrokeWeight="1" GradientFillStart="0 0" GradientFillLength="0" GradientFillAngle="0" GradientStrokeStart="0 0" GradientStrokeLength="0" GradientStrokeAngle="0" Locked="false" LocalDisplaySetting="Default" GradientFillHiliteLength="0" GradientFillHiliteAngle="0" GradientStrokeHiliteLength="0" GradientStrokeHiliteAngle="0" AppliedObjectStyle="ObjectStyle/$ID/[None]" Visible="true" Name="$ID/" ItemTransform="1 0 0 1 36 -8.264665355094508">
          <Properties>
            <PathGeometry>
              <GeometryPathType PathOpen="false">
                <PathPointArray>
                  <PathPointType Anchor="-36 -36" LeftDirection="-36 -36" RightDirection="-36 -36" />
                  <PathPointType Anchor="-36 8.264665355094506" LeftDirection="-36 8.264665355094506" RightDirection="-36 8.264665355094506" />
                  <PathPointType Anchor="32.03149606299213 8.264665355094506" LeftDirection="32.03149606299213 8.264665355094506" RightDirection="32.03149606299213 8.264665355094506" />
                  <PathPointType Anchor="32.03149606299213 -36" LeftDirection="32.03149606299213 -36" RightDirection="32.03149606299213 -36" />
                </PathPointArray>
              </GeometryPathType>
            </PathGeometry>
...
<?xpacket end="r"?>]]></Contents>
      ...
      <Link Self="u10cd8" AssetURL="$ID/" AssetID="$ID/" LinkResourceURI="{$linkUri2}" LinkResourceFormat="$ID/{$extension}" StoredState="Normal" LinkClassID="35906" LinkClientID="257" LinkResourceModified="false" LinkObjectModified="false" ShowInUI="true" CanEmbed="true" CanUnembed="true" CanPackage="true" ImportPolicy="NoAutoImport" ExportPolicy="NoAutoExport" LinkImportStamp="file 130543248448593825 52461" LinkImportModificationTime="2014-09-04T13:20:44" LinkImportTime="2014-09-09T15:08:45" LinkResourceSize="0~cced" />
  ...
  <xsl:template match="*[df:class(., 'topic/image')][@placement='inline'][ancestor::*[df:class(., 'topic/entry')]]" priority="25">
    <!-- FIXME: The Link URL can be relative as long as it still starts
         with file:/ (and CS6 and older only supports file:/ URLs as far
         as I can determine).
      ...
      <Properties>
        <PathGeometry>
          <GeometryPathType PathOpen="false">
            <PathPointArray>
              <PathPointType Anchor="-6 -6" LeftDirection="-6 -6" RightDirection="-6 -6" />
              <PathPointType Anchor="-6 51.72763787140658" LeftDirection="-6 51.72763787140658" RightDirection="-6 51.72763787140658" />
              <PathPointType Anchor="95.4724409448819 51.72763787140658" LeftDirection="95.4724409448819 51.72763787140658" RightDirection="95.4724409448819 51.72763787140658" />
              <PathPointType Anchor="95.4724409448819 -6" LeftDirection="95.4724409448819 -6" RightDirection="95.4724409448819 -6" />
            </PathPointArray>
          </GeometryPathType>
        </PathGeometry>
      </Properties>
        ...
        <PathGeometry>
          <GeometryPathType PathOpen="false">
            <PathPointArray>
              ...
              <PathPointType Anchor="-72 260.559055128042" LeftDirection="-72 260.559055128042"
                RightDirection="-72 260.559055128042"/>
              <PathPointType Anchor="381.5433070864329 260.559055128042"
                LeftDirection="381.5433070864329 260.559055128042"
                RightDirection="381.5433070864329 260.559055128042"/>
              <PathPointType Anchor="381.5433070864329 -47" LeftDirection="381.5433070864329 -47"
                RightDirection="381.5433070864329 -47"/>
            </PathPointArray>
          </GeometryPathType>
        </PathGeometry>
      </Properties>
