...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    exclude-result-prefixes="xs opentopic-func exslf exsl dita2xslfo mml" version="2.0">
    ...
    <xsl:template match="*[contains(@class, ' topic/dl ')]">
        ...
        <xsl:choose>
            ...
            <xsl:when test="($nested_insert_start = 'odd' and $nested_insert_end = 'even') or
                ...
                <fo:block xsl:use-attribute-sets="p common.border__right">
                    ...
                    <xsl:comment>dl block</xsl:comment>
                    <xsl:choose>
                        <xsl:when test="parent::*[contains(@class, 'topic/bodydiv')][@outputclass = 'definitions_level_1'] or
                            parent::*[contains(@class, 'topic/bodydiv')][@outputclass = 'definition_with_note'] or
                            parent::*[contains(@class, 'topic/bodydiv')][@outputclass = 'definitions_level_2_bullet'] or
                            parent::*[contains(@class, 'topic/bodydiv')][@outputclass = 'definitions_level_2_dash']">
                            <fo:block space-after="6pt" space-before="6pt">
                                <!-- for the IET Defintion templates see lists.xsl -->
                                <xsl:apply-templates mode="IET_definitions"/>
                            </fo:block>
                        </xsl:when>
                        <xsl:when test="parent::*[contains(@class, 'topic/bodydiv')][@outputclass = 'symbol_information']">
                            <fo:block>
                                <xsl:if test="parent::*/descendant::processing-instruction('oxy_insert_start') or
                                    parent::*/descendant::processing-instruction('oxy_delete') or
                                    parent::*/descendant::processing-instruction('oxy_comment_start')">
                                    <xsl:attribute name="border-right">1pt solid black</xsl:attribute>
                                </xsl:if>
                            <fo:table inline-progression-dimension="95%" table-layout="fixed">                                
                                <xsl:call-template name="commonattributes"/>
                                ...
                                <fo:table-body>                       
                                    <!-- for the IET Symbol templates see lists.xsl -->
    ...
    <xsl:template match="*" mode="get-propvalue-position">
        <xsl:choose>
            <xsl:when
                test="../*[contains(@class, ' reference/property ')]/*[contains(@class, ' reference/proptype ')] |
    ...
    <xsl:template match="*" mode="get-propdesc-position">
        <xsl:choose>
            <xsl:when
                test="../*/*[contains(@class, ' reference/proptype ') or contains(@class, ' reference/proptypehd ')] and
                        ../*/*[contains(@class, ' reference/propvalue ') or contains(@class, ' reference/propvaluehd ')]">
                <xsl:text>3</xsl:text>
            </xsl:when>
            <xsl:when
                test="../*/*[contains(@class, ' reference/proptype ') or contains(@class, ' reference/proptypehd ')] |
                        ../*/*[contains(@class, ' reference/propvalue ') or contains(@class, ' reference/propvaluehd ')]">
                <xsl:text>2</xsl:text>
            </xsl:when>
            <xsl:otherwise>1</xsl:otherwise>
    ...
    <xsl:template match="*[contains(@class, ' reference/property ')]">
        <fo:table-row xsl:use-attribute-sets="property">
            ...
            <xsl:variable name="frame">
                <xsl:variable name="f"
                    select="ancestor::*[contains(@class, ' reference/properties ')][1]/@frame"/>
                <xsl:choose>
                    <xsl:when test="$f">
                        <xsl:value-of select="$f"/>
                    </xsl:when>
            ...
            <xsl:variable name="keyCol"
                select="number(ancestor::*[contains(@class, ' reference/properties ')][1]/@keycol)"/>
            ...
            <xsl:choose>
                ...
                <xsl:when
                    test="../*/*[contains(@class, ' reference/propvalue ') or contains(@class, ' reference/propvaluehd ')]">
                    <xsl:call-template name="createEmptyPropertyEntry">
                        <xsl:with-param name="entryCol" select="$valuePos"/>
                        <xsl:with-param name="keyCol" select="$keyCol"/>
                        <xsl:with-param name="hasHorisontalBorder" select="$hasHorisontalBorder"/>
            ...
            <xsl:choose>
                ...
                <xsl:when
                    test="../*/*[contains(@class, ' reference/propdesc ') or contains(@class, ' reference/propdeschd ')]">
                    <xsl:call-template name="createEmptyPropertyEntry">
                        <xsl:with-param name="entryCol" select="$descPos"/>
                        <xsl:with-param name="keyCol" select="$keyCol"/>
                        <xsl:with-param name="hasHorisontalBorder" select="$hasHorisontalBorder"/>
    ...
    <xsl:template
        match="*[contains(@class, ' reference/proptype ') or contains(@class, ' reference/propvalue ') or contains(@class, ' reference/propdesc ')]">
        <xsl:param name="entryCol"/>
        <fo:table-cell xsl:use-attribute-sets="property.entry">
            <xsl:call-template name="commonattributes"/>
            <xsl:variable name="frame">
            ...
            <xsl:if
                test="following-sibling::*[contains(@class, ' reference/proptype ') or contains(@class, ' reference/propvalue ') or contains(@class, ' reference/propdesc ')]">
                <xsl:call-template name="generateSimpleTableVerticalBorders">
                    <xsl:with-param name="frame" select="$frame"/>
                </xsl:call-template>
            </xsl:if>
            ...
            <xsl:choose>
                <xsl:when
                    test="number(ancestor::*[contains(@class, ' reference/properties ')][1]/@keycol) = $entryCol">
    ...
    <xsl:template
        match="*[contains(@class, ' reference/proptypehd ') or contains(@class, ' reference/propvaluehd ') or contains(@class, ' reference/propdeschd ')]">
        <xsl:param name="entryCol"/>
        <fo:table-cell xsl:use-attribute-sets="prophead.entry">
            <xsl:call-template name="commonattributes"/>
            <xsl:variable name="frame">
            ...
            <xsl:if
                test="following-sibling::*[contains(@class, ' reference/proptypehd ') or contains(@class, ' reference/propvaluehd ') or contains(@class, ' reference/propdeschd ')]">
                <xsl:call-template name="generateSimpleTableVerticalBorders">
                    <xsl:with-param name="frame" select="$frame"/>
                </xsl:call-template>
            </xsl:if>
    ...
    <xsl:template
        match="*[contains(@class, ' topic/table ')][@outputclass='landscape'][not(parent::*[contains(@class, ' topic/bodydiv ')][@outputclass='table_with_note'])]"
        priority="10">
        <fo:block break-before="page" break-after="page"> </fo:block>
    </xsl:template>

    <xsl:template match="*[contains(@class, ' topic/table ')]" mode="secondPass">
    ...
    <xsl:template match="*[contains(@class, ' topic/tbody ')]/*[contains(@class, ' topic/row ')]">
        ...
        <fo:table-row>
            <!-- SA00 table footnotes -->
            <xsl:choose>
                <xsl:when
                    test="not(descendant::*[contains(@class, ' topic/p ')][@outputclass='table_footnote']) and not(ancestor::*[contains(@class, ' topic/table ')][@frame='none'])">
             ...
             <xsl:call-template name="commonattributes"/>
            <xsl:apply-templates>
                <xsl:with-param name="row_nested_insert_start" select="$nested_insert_start"/>
                <xsl:with-param name="row_nested_insert_end" select="$nested_insert_end"/>                
                <xsl:with-param name="row_comments_and_deletes" select="$comments_and_deletes"/>
                <xsl:with-param name="row_comments_and_deletes_content">
                    <xsl:copy-of select="$comments_and_deletes_content"/>
                </xsl:with-param>
            </xsl:apply-templates>
        </fo:table-row>
    </xsl:template>
    ...
    <xsl:template
        ...
        <xsl:param name="row_nested_insert_start" />
        <xsl:param name="row_nested_insert_end" />                
        <xsl:param name="row_comments_and_deletes" />
        <xsl:param name="row_comments_and_deletes_content"/>
        <!-- determine if there is an odd or even number of insert start PIs on the
            preceding-sibling axis -->
        <xsl:variable name="nested_insert_start" >
            <xsl:call-template name="determine_nested_insert_start"/>
        ...
        <fo:table-cell xsl:use-attribute-sets="thead.row.entry">
            ...
            <xsl:if
                test="following-sibling::*[contains(@class, ' topic/entry ')] and not(ancestor::*[contains(@class, ' topic/table ')][@frame='none'])">
            <xsl:attribute name="border-right-style">solid</xsl:attribute>
            <xsl:attribute name="border-right-width">.5pt</xsl:attribute>
            <xsl:attribute name="border-right-color">black</xsl:attribute>
            </xsl:if>
            <!-- if the row for this element has any change marks that come immediately before it, process them
            here -->
            <!-- going to disable this: it's only gong to cause an FO parse error and trasform will fail
            there's really nothing to do with deletes or comments that are between rows: they don't
            fit anywhere -->
           <!-- <xsl:if test="$row_comments_and_deletes = 'change mark for this element'">           
                <xsl:apply-templates select="$row_comments_and_deletes_content/processing-instruction()"
                    mode="apply_change_marks_to_block"/>
            </xsl:if>-->               
            <!-- if this element has any change marks that come immediately before it, process them
            here -->
            ...
            <xsl:choose>
                <xsl:when test="child::processing-instruction('oxy_insert_start') or 
                    ...
                    <fo:block xsl:use-attribute-sets="thead.row.entry__content  common.border__right">
                        <xsl:if test="($row_nested_insert_start = 'odd' and $row_nested_insert_end = 'even') or 
                            ($nested_insert_start = 'odd' and $nested_insert_end = 'even')">
                            <xsl:attribute name="color">green</xsl:attribute>
                            <xsl:attribute name="font-weight">bold</xsl:attribute>
                        </xsl:if>
    ...
    <xsl:template
       ...
       <!-- <xsl:if test="$row_comments_and_deletes = 'change mark for this element'">           
        ...
        <xsl:choose>
            <xsl:when test="not(following-sibling::*[contains(@class, ' topic/entry ')])">
                <xsl:choose>
                    <xsl:when
                        test="preceding-sibling::*[contains(@class, ' topic/entry ')]/child::processing-instruction('oxy_insert_start') or
                        preceding-sibling::*[contains(@class, ' topic/entry ')]/child::processing-instruction('oxy_delete') or 
                        preceding-sibling::*[contains(@class, ' topic/entry')]/child::processing-instruction('oxy_comment_start') or
                        child::processing-instruction('oxy_insert_start') or 
                        child::processing-instruction('oxy_delete') or
                        child::processing-instruction('oxy_comment_start')">
            ...
            here -->
                            ...
                            <fo:block
                                xsl:use-attribute-sets="tbody.row.entry__content  common.border__right">
                                <xsl:if test="($row_nested_insert_start = 'odd' and $row_nested_insert_end = 'even') or 
                                    ($nested_insert_start = 'odd' and $nested_insert_end = 'even')">
                                    <xsl:attribute name="border-right">1pt solid black</xsl:attribute>
                                    <xsl:attribute name="color">green</xsl:attribute>                                    
            ...
            <xsl:otherwise>
                <fo:table-cell xsl:use-attribute-sets="tbody.row.entry">
                    ...
                    <xsl:call-template name="generateTableEntryBorder"/>
                    <xsl:if
                        test="not(ancestor::*[contains(@class, ' topic/table ')][@frame='none'])">
                        <xsl:attribute name="border-right-style">solid</xsl:attribute>
                        <xsl:attribute name="border-right-width">.5pt</xsl:attribute>
                        <xsl:attribute name="border-right-color">black</xsl:attribute>
                    </xsl:if>
    ...
    <xsl:template name="processEntryContent">
        ...
        <xsl:choose>
            ...
            <xsl:otherwise>
                <xsl:choose>
                    <xsl:when test="descendant::processing-instruction('oxy_insert_start') or 
