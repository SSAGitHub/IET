...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	exclude-result-prefixes="opentopic-mapmerge opentopic-func exslf exsl mml" version="2.0">
	...
	<xsl:template match="*[processing-instruction()[name()='ditaot'][.='usertext']]"
		...
		<xsl:apply-templates select="*[not(contains(@class,' topic/desc '))]|text()" mode="filter_for_PIs"/>
	</xsl:template>
			...
			<xsl:when test="not($element) or ($destination = '') or $referenceContent='#none#'">
				<xsl:choose>
					<xsl:when test="*[not(contains(@class,' topic/desc '))] | text()">
						...
						<xsl:apply-templates
							select="*[not(contains(@class,' topic/desc '))] | text()" mode="filter_for_PIs"/>
					</xsl:when>
					<xsl:otherwise><xsl:message>------------------- @@@@@@@@@@@@@@@@@@@ ----------------------------------</xsl:message>
						<xsl:value-of select="$href"/>
					</xsl:otherwise>
				</xsl:choose>
			</xsl:when>

			<xsl:otherwise><xsl:message>------------------- ZZZZZZZZZZZZZZ ----------------------------------</xsl:message>
	...
	<xsl:template match="*[contains(@class, ' topic/fig ')][*[contains(@class, ' topic/title ')]]"
		...
		<xsl:call-template name="insertVariable">
			...
			<xsl:with-param name="theParameters">
				<number>
					<xsl:value-of
						select="count(preceding::*[contains(@class, ' topic/fig ')][child::*[contains(@class, ' topic/title ')]]) + 1"
					/>
				</number>
				<title>
	...
	<xsl:template match="*[contains(@class,' topic/xref ')][not(@href)]" priority="20">
		<fo:block xsl:use-attribute-sets="xref">
			<xsl:apply-templates/>
		</fo:block>
	</xsl:template>
	...
	<xsl:template match="*[contains(@class,' topic/link ')]">
		...
		<xsl:choose>
			<xsl:when
				test="(@role and not(contains($includeRelatedLinkRoles, concat(' ', @role, ' ')))) or
                        (not(@role) and not(contains($includeRelatedLinkRoles, ' #default ')))"/>
			<xsl:when
				test="@role='child' and $chapterLayout='MINITOC' and
...
-->
	...
	<xsl:template name="buildBasicLinkDestination">
		...
		<xsl:choose>
			<xsl:when
				test="(contains(@href, '://') and not(starts-with(@href, 'file://')))
