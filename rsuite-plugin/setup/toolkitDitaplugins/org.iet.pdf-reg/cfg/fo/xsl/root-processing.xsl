...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    exclude-result-prefixes="opentopic-index opentopic opentopic-i18n opentopic-func dita-ot xs mml"
    version="2.0">
    
    <xsl:param name="bookmap-order" select="'discard'"/>
  
  <xsl:variable name="map_contents">
    <xsl:for-each select="*[contains(@class, ' map/map ')][not(ancestor::*)]">
      <xsl:copy-of select="."/>
    </xsl:for-each>
  </xsl:variable>
  ...
  <xsl:variable name="topicFigContent">
    <xsl:for-each select="//*[contains(@class, ' topic/topic ')]">
      <topic guid="{generate-id()}">
        <xsl:call-template name="commonattributes"/>
        <xsl:copy-of select="descendant::*[contains(@class, ' topic/fig ')][*[contains(@class, ' topic/title ')]]"/>
      </topic>
    </xsl:for-each>
  </xsl:variable>
  
  <xsl:variable name="topicTableContent">
    <xsl:for-each select="//*[contains(@class, ' topic/topic ')]">
      <topic guid="{generate-id()}">
        <xsl:call-template name="commonattributes"/>
        <xsl:copy-of select="descendant::*[contains(@class, ' topic/table ')][*[contains(@class, ' topic/title ')]]"/>
      </topic>
    </xsl:for-each>
  </xsl:variable>  
...
<!-- Root template, and topicref validation mooved from topic2fo_shell.xsl to add ability for customizaing   -->
 ...
 <!--     <xsl:message>____________ ^^^^^^^^^^^^^^^^^^^ ######## <xsl:copy-of
   select="$topicref_order"/></xsl:message>-->
